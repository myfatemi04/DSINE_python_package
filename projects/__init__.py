...
EXPERIMENT_DIR = "/Users/michaelfatemi/Code/2024/Computer_Vision_Project/external/DSINE/projects/dsine/experiments"  # where to save the experiments     NOTE: this should be changed!
...
def get_default_parser():
    ...
    parser.add_argument(
        "--gpus", type=str, default="-1", help="which gpus to use, if -1, use all"
    )
    parser.add_argument("--save_all_models", action="store_true")
    parser.add_argument(
        "--overwrite_models",
        action="store_true",
        help="if True, overwrite the existing checkpoints",
    )
    ...
    parser.add_argument(
        "--diff_lr",
        action="store_true",
        help="use different LR for different network components",
    )
    ...
    parser.add_argument(
        "--visualize_every", default=1000, type=int, help="visualize every N iterations"
    )
    ...
    parser.add_argument(
        "--data_augmentation_intrins", default=False, action="store_true"
    )
    ...
    parser.add_argument(
        "--data_augmentation_persp_random_fov", default=False, action="store_true"
    )
    ...
    parser.add_argument(
        "--data_augmentation_nyu_crop", default=False, action="store_true"
    )
