...
from dsine.utils.projection import intrins_from_fov
from dsine.utils.utils import get_padding
...
class InputScreen:
    def __init__(
        ...
        **kwargs
        ...
        if intrins is None:
            self.intrins = intrins_from_fov(
                new_fov=60.0, H=self.new_H, W=self.new_W, device=device
            )
        else:
    ...
    def get_sample(self):
        ...
        if self.interp is not None:
            color_image = cv2.resize(
                color_image, (self.new_W, self.new_H), interpolation=self.interp
            )
        ...
        **kwargs
        ...
        for s in rs_device.sensors:
            if s.get_info(rs.camera_info.name) == "RGB Camera":
                ...
                s.set_option(
                    rs.option.enable_auto_white_balance, enable_auto_white_balance
                )
                break
        if not found_rgb:
...
class InputYoutube:
    ...
    def init_stream(self):
        ...
        self.stream = CamGear(
            source="https://youtu.be/%s" % self.video_id, stream_mode=True, logging=True
        ).start()
