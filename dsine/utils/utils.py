...
def load_checkpoint(fpath, model):
    ...
    ckpt = torch.load(fpath, map_location="cpu", weights_only=True)["model"]
...
class RunningAverage:
    ...
    def append(self, value, count_add=1):
        self.avg = (count_add * value + self.count * self.avg) / (
            count_add + self.count
        )
...
def pad_input(img, intrins, lrtb=(0, 0, 0, 0)):
    ...
    if l + r + t + b != 0:
        ...
        img_R = F.pad(
            img[:, 0:1, :, :], (l, r, t, b), mode="constant", value=pad_value_R
        )
        img_G = F.pad(
            img[:, 1:2, :, :], (l, r, t, b), mode="constant", value=pad_value_G
        )
        img_B = F.pad(
            img[:, 2:3, :, :], (l, r, t, b), mode="constant", value=pad_value_B
        )
