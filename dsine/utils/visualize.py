...
from dsine.utils.utils import compute_normal_error
...
def unnormalize(
    img_in, img_stats={"mean": [0.485, 0.456, 0.406], "std": [0.229, 0.224, 0.225]}
):
...
def kappa_to_alpha(pred_kappa, to_numpy=True):
    ...
    if torch.is_tensor(pred_kappa):
        alpha = ((2 * pred_kappa) / ((pred_kappa**2.0) + 1)) + (
            (torch.exp(-pred_kappa * np.pi) * np.pi)
            / (1 + torch.exp(-pred_kappa * np.pi))
        )
...
def visualize_normal(
    ...
    for i in range(num_vis):
        ...
        if gt_norm is not None:
            ...
            plt.imsave(
                target_path, normal_to_rgb(gt_norm[i, ...], gt_norm_mask[i, ...])
            )
...
def visualize_normal_tb(args, img, norm_out, gt_norm, gt_norm_mask):
    ...
    if pred_kappa is not None:
        if "NLL_angmf" in args.loss_fn:
            vis_list.append(
                depth_to_rgb(
                    kappa_to_alpha(pred_kappa[0, ...]), None, d_min=0.0, d_max=error_max
                )
            )
        else:
            vis_list.append(
    ...
    if gt_norm is not None:
        ...
        vis_list.append(
            depth_to_rgb(
                pred_error[0, ...], gt_norm_mask[0, ...], d_min=0.0, d_max=error_max
            )
        )
