...
import dsine.utils.visualize as vis_utils
...
import dsine.projects.baseline_normal.config as config
from dsine.projects.baseline_normal.dataloader import *
from dsine.projects.baseline_normal.losses import ComputeLoss
...
def train(model, args, device):
    ...
    if not args.diff_lr:
        ...
        params = [
            {"params": m.n_net.model.get_1x_lr_params(), "lr": args.lr / 10},
    ...
    for epoch in range(args.num_epochs):
        ...
        if args.rank == 0:
            data_loader = tqdm(
                train_loader, desc=f"Epoch: {epoch + 1}/{args.num_epochs}. Loop: Train"
            )
        else:
        ...
        for batch_idx, data_dict in enumerate(data_loader):
            ...
            if should_write and (
                (total_iter % args.visualize_every) < args.batch_size_orig
            ):
                vis_ = vis_utils.visualize_normal_tb(
                    args, img, norm_out, gt_norm, gt_norm_mask
                )
                args.writer.add_image(
                    "train vis", vis_, global_step=total_iter, dataformats="HWC"
                )
            # ↑↑↑↑

            # validation
            if should_write and (
                (total_iter % args.validate_every) < args.batch_size_orig
            ):
                if args.save_all_models:
                    utils.save_model(
                        model,
                        os.path.join(
                            args.output_dir, "models", "iter_%010d.pt" % total_iter
                        ),
                        total_iter,
                    )
                else:
                    ...
                    if metrics[BEST_KEY] <= best_acc:
                        utils.save_model(
                            model,
                            os.path.join(args.output_dir, "models", "best.pt"),
                            total_iter,
                        )
        ...
        if should_write:
            if args.save_all_models:
                ...
                if epoch + 1 == args.num_epochs:
                    utils.save_model(
                        model,
                        os.path.join(args.output_dir, "models", "last.pt"),
                        total_iter,
                    )
                model.train()
...
def validate(model, args, val_loader, device, total_iter):
    with torch.no_grad():
        ...
        for data_dict in tqdm(val_loader, desc="Loop: Validation"):
            ...
            metrics_, num_pixels = utils.compute_normal_metrics2(
                pred_error[gt_norm_mask]
            )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # ↓↓↓↓
    # NOTE: define model
    from dsine.projects.baseline_normal.model import NNET
    ...
    if args.distributed:
        ...
        logger.info(
            "GPU: %s / RANK: %s / Batch size: %s / Num workers: %s"
