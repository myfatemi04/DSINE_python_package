        ...
        for data_dict in tqdm(test_loader):
            ...
            if "normal" in data_dict.keys():
                ...
                if total_normal_errors is None:
                    ...
                    total_normal_errors = torch.cat(
                        (total_normal_errors, pred_error[gt_norm_mask]), dim=0
                    )
...
def run_on_folder(args, model, device, input_folder: str, output_folder: str):
    img_paths = glob.glob(input_folder + "/*.png") + glob.glob(input_folder + "/*.jpg")
    img_paths.sort()
    ...
    while True:
        with torch.no_grad():
            if pause:
                ...
                t1 = time.time()
                print(f"Forward pass complete. Took {t1-t0:.3f}s.")
                ...
                if pred_kappa.size(1) == 0:
                    ...
                    pred_uncertainty = [
                        vis_utils.alpha_to_jet(vis_utils.kappa_to_alpha(pred_kappa))
                    ]  # BGR
                else:
                    pred_uncertainty = [
                        vis_utils.depth_to_rgb(
                            pred_kappa[0, ...], None, 0.0, None, "gray"
                        )[..., ::-1]
                    ]
...
if __name__ == "__main__":
    ...
    # NOTE: measure throughput
        ...
        dummy_intrins = (
            torch.cat([intrins_from_fov(60.0, H, W).unsqueeze(0)] * batch_size, dim=0)
            .float()
            .to(device)
        )
    ...
    # demo
        ...
        from dsine.utils.demo_data import define_input
