...
from dsine.projects import DATASET_DIR, EXPERIMENT_DIR, get_default_parser
...
def make_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Input folder when inferencing on a video",
    )
    parser.add_argument(
        "--output_folder",
        type=str,
        default="",
        help="Output folder when inferencing on a video",
    )
    # ↑↑↑↑
    return parser
...
def get_args(test=False):
    ...
    os.makedirs(
        os.path.join(args.output_dir, "test"), exist_ok=True
    )  # save test images

    if (
        not test
        and not args.overwrite_models
        and len(glob.glob(os.path.join(args.output_dir, "models", "*.pt"))) > 0
    ):
    ...
    if not test:
        global logger
        utils.change_logger_dest(
            logger, os.path.join(args.output_dir, "log", "%s.log" % datetime.now())
        )
