...
import dsine.data.augmentations.basic as aug_basic
import dsine.data.augmentations.appearance as aug_appear
import dsine.data.augmentations.perspective as aug_persp
...
def get_transform(args, dataset_name="hypersim", mode="train"):
    ...
    if mode == "train" and args.data_augmentation_intrins:
        logger.info(
            "Augmentation - Randomize intrinisics (randomly resize & crop the input image)"
        )
        tf_list += [aug_basic.RandomIntrins()]
        ...
        logger.info(
            "Preprocessing - Resize to H, W = %s, %s"
        ...
        logger.info(
            "Preprocessing - Set the field-of-view to %s degrees (H, W = %s, %s)"
            % (args.data_augmentation_same_fov, args.input_height, args.input_width)
        )
        tf_list += [
            aug_persp.SameFov(
                new_fov=args.data_augmentation_same_fov,
                H=args.input_height,
                W=args.input_width,
            ),
        ]
    if mode == "train" and args.data_augmentation_crop:
        logger.info(
            "Augmentation - Random crop to H, W = %s, %s"
            % (args.data_augmentation_crop_height, args.data_augmentation_crop_width)
        )
        tf_list += [
            aug_basic.RandomCrop(
                height=args.data_augmentation_crop_height,
    ...
    if mode == "train" and args.data_augmentation_appear > 0:
        logger.info(
            "Augmentation - Appearance augmentation (ver %s)"
            % args.data_augmentation_appear
        )
        tf_list += [
