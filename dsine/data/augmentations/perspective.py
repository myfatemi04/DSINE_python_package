...
from dsine.utils.rotation import get_R
from dsine.utils.projection import (
    get_ray_array,
    intrins_from_fov,
    pix_to_src_coords,
    zbuffer_to_radial,
...
class SameFov:
    ...
    def __call__(self, sample):
        ...
        # (3, H*W)
        tgt_ray = get_ray_array(
            H=self.new_H, W=self.new_W, intrins=self.new_intrins.unsqueeze(0)
        ).squeeze(
            0
        )  # (3, H*W)
        src_pix = sample.intrins.matmul(tgt_ray)  # (3, H*W)
        src_coords = pix_to_src_coords(
            src_pix, new_H=self.new_H, new_W=self.new_W, orig_H=H, orig_W=W
        )
        ...
        if sample.depth_mask is not None:
            depth_mask_warped = (
                F.grid_sample(
                    sample.depth_mask.float().unsqueeze(0),
                    src_coords,
                    mode="nearest",
                    padding_mode="zeros",
                    align_corners=False,
        ...
        if sample.normal is not None:
            normal_warped = F.grid_sample(
                sample.normal.unsqueeze(0),
                src_coords,
                mode="nearest",
                padding_mode="zeros",
                align_corners=False,
        ...
        if sample.normal_mask is not None:
            normal_mask_warped = (
                F.grid_sample(
                    sample.normal_mask.float().unsqueeze(0),
                    src_coords,
                    mode="nearest",
                    padding_mode="zeros",
                    align_corners=False,
...
class RotationAndScale:
    ...
    def __call__(self, sample):
        if random.random() < self.p:
            ...
            # (1, H, W, 2)
            src_coords = pix_to_src_coords(
                src_pix, new_H=new_H, new_W=new_W, orig_H=orig_H, orig_W=orig_W
            )
            ...
            if sample.depth is not None:
                radial = zbuffer_to_radial(
                    sample.depth.unsqueeze(0),
                    sample.intrins.unsqueeze(0),
                    orig_H,
                    orig_W,
                )
                radial_warped = F.grid_sample(
                    radial,
                    src_coords,
                    mode="nearest",
                    padding_mode="zeros",
                ...
                depth_warped = radial_to_zbuffer(
                    radial_warped, new_intrins.unsqueeze(0), new_H, new_W
                ).squeeze(0)
                sample.depth = depth_warped
            ...
            if sample.normal is not None:
                ...
                normal_warped = R.matmul(
                    normal_warped.squeeze(0).reshape(3, -1)
                ).reshape(3, new_H, new_W)
                sample.normal = normal_warped
