...
from dsine.models.conv_encoder_decoder.submodules import (
    Encoder,
    UpSampleBN,
    UpSampleGN,
    INPUT_CHANNELS_DICT,
...
from dsine.models.dsine.submodules import normal_activation
...
class DSINE_v00(nn.Module):
    def __init__(self, args):
        ...
        logger.info(
            "output_dim: %s / down: %s / learned upsampling: %s"
            % (args.NNET_output_dim, down, learned_upsampling)
        )
        self.encoder = Encoder(B=B, pretrained=True)
        self.decoder = Decoder(
            num_classes=args.NNET_output_dim,
            B=B,
            NF=NF,
            BN=BN,
            down=down,
...
class Decoder(nn.Module):
    def __init__(
        self, num_classes=3, B=5, NF=2048, BN=False, down=8, learned_upsampling=True
    ):
        ...
        self.conv2 = nn.Conv2d(
            input_channels[0], features, kernel_size=1, stride=1, padding=0
        )
        self.up1 = UpSample(
            skip_input=features // 1 + input_channels[1],
            output_features=features // 2,
            align_corners=False,
        )
        self.up2 = UpSample(
            skip_input=features // 2 + input_channels[2],
        ...
        if down == 8:
            ...
            self.up3 = UpSample(
                skip_input=features // 4 + input_channels[3],
            ...
            self.up4 = UpSample(
                skip_input=features // 8 + input_channels[4],
        ...
        if learned_upsampling:
            self.mask_head = get_prediction_head(
                i_dim, 128, 9 * self.downsample_ratio * self.downsample_ratio
            )
