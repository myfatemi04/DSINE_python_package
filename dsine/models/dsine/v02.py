...
from dsine.models.dsine.submodules import get_pixel_coords, ConvGRU, get_unfold, RayReLU
from dsine.utils.rotation import axis_angle_to_matrix
...
class DSINE_v02(nn.Module):
    def __init__(self, args):
        ...
        logger.info(
            "B: %s / NF: %s / BN: %s / down: %s / learned_upsampling: %s"
            % (B, NF, BN, down, learned_upsampling)
        )
        logger.info(
            "ps: %s / iter train: %s / iter test: %s / v_relu: %s"
            % (self.ps, self.num_iter_train, self.num_iter_test, self.v_relu)
        )
